...
// Bond Pricer
double bond::Bond_Pricer(double time_to_maturity, double interest_rate, double coupon_rate, double frequency, double face_value = 100) const {
...
double bond::average(vector<double> v) const
{
...
double bond::special_BP(string start_date, string end_date, double CR, double freq, double face_value) {
    ...
    int diff = DaysToYear(DateDiff(start_date, end_date)) - 1;
...
double bond::PV_special_BP(string purchase_date, string end_date, double sp_BP){
    ...
    double ttm = DateDiff(purchase_date, end_date);
